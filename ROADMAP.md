...
**Phase:** 12 — Desktop Native Integrations — 🟨 STARTING (spike-ordered: SMTC spike → tray → mini-player → shortcuts → close-to-tray/geometry → jpackage). Phases 10+11 ✅ **MERGED** — code + unit tests in `main`; **on-hardware verification OPEN** (physical Android/desktop run, like the Phases 08/09 checklists).

**Files this session:** verification prep — `docs/verification/11-lyrics.md` gained a live-pre-verified test track list (LRCLIB API checked from sandbox via fetch_page: Queen–Bohemian Rhapsody synced EN/long + blanks, Arijit Singh–Channa Mereya synced HI, PSY–Gangnam Style synced KR, Luis Fonsi–Despacito synced ES, YOASOBI–夜に駆ける unsynced JP; 3 NE candidates 404 = coverage finding). Phase 12 code (next): `app-desktop` tray (AWT SystemTray), mini-player window (320×88 always-on-top), keyboard shortcuts, close-to-tray + window geometry persistence, SMTC spike (JNA/WinRT, feature-flagged), `docs/verification/12-desktop-native.md`.
...
**Exact next step:** Phase 12 build per MASTER_PROMPT spike order: (1) SMTC spike (3-day timebox — JNA/WinRT probe + metadata/state push; if unstable on the user's Windows box → documented fallback = tray + focused-window media keys, per RISK_REGISTER), (2) system tray (icon playing/paused variants, menu: track / play-pause / next / prev / open / quit), (3) mini-player window (320×88 always-on-top, draggable, click opens main), (4) keyboard shortcuts (Space, ←/→ seek 5 s, Ctrl+←/→ prev/next, Ctrl+F search, Ctrl+M mini-player, Ctrl+Q quit), (5) close-to-tray (default on, persisted via `SettingsKeys.CLOSE_TO_TRAY`) + window geometry (`SettingsKeys.WINDOW_GEOMETRY`), (6) jpackage packaging + clean-VM install test. Write `docs/verification/12-desktop-native.md` from the code. In parallel: user runs the Phase 10+11 on-hardware checklists (concrete track list ready in `docs/verification/11-lyrics.md`).
...
| 12 | Desktop native (SMTC spike, tray, mini-player, jpackage) | 🟨 STARTING this session — spike-ordered build in `app-desktop` (tray, mini-player window, shortcuts, close-to-tray + geometry, SMTC JNA spike feature-flagged, jpackage) | docs/verification/12-desktop-native.md (to write) |
