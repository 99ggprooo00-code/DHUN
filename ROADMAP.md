...
**Phase:** 12 — Desktop Native Integrations — 🟨 IN PROGRESS (all code pushed; CI compile gate RED ×3 — 1.8.2 API fixes in progress; hardware OPEN). Phases 10+11 ✅ **MERGED** — **on-hardware verification OPEN** (user's physical run; concrete tracks ready in `docs/verification/11-lyrics.md`).
...
**Fixes pushed so far:** `f50dba4` (round 1) + `d2e9ade` (round 2 — window handles via **public JDK `java.awt.Frame.getFrames()`** exact-title lookup; `Frame.isVisible/toFront/requestFocus/x/y/width/height` for close-to-tray / tray "Open" / Ctrl+M / geometry; JNA only for SMTC probe + Windows in-content drag).

**Exact next step (round 3 — diagnose before guessing; no more API guesswork):**
1. Pull the **full untruncated** annotations for run `33930616806`: `gh api repos/99ggprooo00-code/DHUN/check-runs/101208278583/annotations --jq '.[].message'` — the `None of the following candidates` bodies list the exact `rememberWindowState`/`Window` overload signatures (incl. the `WindowPosition` type). **If the sandbox GH token is dead again (401), reconnect GitHub in Arena first.**
2. Cross-check the 1.8.2 API shape from the public repo's **1.8.2 CHANGELOG.md** (fetched via fetch_page — the 1.8.0 section documents the window-state/key API with code samples; the desktop `ui` sources themselves are NOT in the public repo at this tag — verified).
3. Fix round 3 in `Main.kt`: arrow-key representation (per the confirmed API), drop/replace the `event.type` check (KeyDown-only may be the default), `position = WindowPosition(…)` per its actual shape (restore from saved x,y).
4. Push → CI → loop until green → **merge PR #9**.
5. **User's Windows machine** (`docs/verification/12-desktop-native.md` checklist): tray (icon variants + 6-item menu), close-to-tray ≠ quit + clean quit (no zombie vlc), window geometry restore, mini-player (always-on-top / drag / click-opens-main / Ctrl+M), all 7 shortcuts + text-field negative check, **`SMTC probe` console line**, `:app-desktop:createMsi` + clean-VM install.
6. Per SMTC probe result → implement spike **phase 2** (`UpdateMetadata` + `ButtonPressed` events; IIDs pulled on-machine per the doc's procedure) or lock in the documented fallback (tray + shortcuts — already the shipping path).
7. In parallel (any machine): user runs the Phase 10+11 on-hardware checklists.
...
### Phase 12 step-by-step status — CURRENT PHASE (🟨 IN PROGRESS — all code pushed `d2e9ade`, CI RED ×3 (`33887658349`/`33928843140`/`33930616806`), round-3 fix pending after full-annotation diagnosis; nothing counts done until CI green + hardware)
...
| System tray: icon playing/paused variants; menu track title / play-pause / next / prev / open / quit | 🟨 **code pushed** (`native/DhunTray.kt` + `native/TrayIcons.kt` — AWT, EDT-marshaled, headless-safe) — CI compile pending (RED ×3 — 1.8.2 API fixes in `f50dba4`+`d2e9ade`, round 3 pending); hardware OPEN |
