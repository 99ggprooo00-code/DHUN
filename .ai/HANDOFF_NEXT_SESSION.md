...
**Ask.** Three visual defects, each fixed on its own terms and explicitly *not* as one polish pass: (1) music started from an album page played with no artwork anywhere — nothing in the player, mini player or shell blur; (2) the album page — then, on the user's follow-up, the artist page too — had no blurred backdrop, unlike Home / Search / Library; (3) the ⋮ overflow menu looked like the old UI and was far too long.

**Constraints honoured (all four, stated before any edit).** No regression to the Android mini player or the accepted Home / Search / Library cards; no lifting the dark surface hexes; no retuning accepted brightness/scrim values; and no discarding the uncommitted edits in `LyricsMaterial.kt` / `FullPlayer.kt` — which did not exist in this checkout: the tree was clean at `03a27b1`, that work is already on `main`, and neither file is touched by this session.

**What landed — three independently reviewable commits.**

1. `cd11056` **fix(browse): album tracks inherit the album cover when rows ship none.** `parseBrowseSongRow` gained a `fallbackThumbnail`; `parseAlbumPage` reads the header cover once (`coverUrl`) and passes it to *both* row-collection paths (the Songs shelf and the tree-wide `collectObjects` fallback), reusing it for `AlbumDetail.thumbnailUrl`. `AlbumTrackRow` shows that art at the playlist-row size (48dp `touchTarget`, `DhunShapes.medium`) beside the track number; the skeleton row matches. New regression fixture `browse-album-anato-no-row-thumbs.json` in **both** fixture roots (the live album fixture with its six row thumbnails removed) + 3 `ParserFixtureTest` cases: rows inherit the cover, real row art is not overwritten, nothing is invented when a page has no artwork anywhere.
2. `f70124f` **ui: album and artist pages sit on their own blurred artwork.** New `PageArtworkBackdrop` is a *wrapper* on `NowPlayingBackdrop`, so the recipe and every pinned number stay `NowPlayingBackdropPolicy`'s (list tier, 64dp blur, dim 0.40, scrim 0.50/0.32/0.44/0.62) — pointed at the page's own artwork instead of the playing track's, with no second set of values to drift. `AlbumScreen` and `ArtistScreen` are transparent and paint it, so both glow while nothing is playing (which is exactly when the shell backdrop has nothing); the album header wash now fades to transparent instead of to opaque background, and the artist parallax hero's fade ends on the existing lyrics-card veil 0.62 so the sharp→blurred seam stays soft and the name stays readable. Fallback rule unchanged: no artwork, or no real blur (`supportsRealtimeBlur`), draws nothing. `PlaylistScreen` was deliberately left opaque — same defect class, out of the ask — and is recorded in `docs/verification/09-browse-pages.md` and `KNOWN_LIMITATIONS.md`.
3. `5d5f77e` **ui: one compact frosted ⋮ menu replaces the old overflow sheet.** `TrackOverflowDialog` — the single dialog mounted once from `DhunAppShell` that serves the full player, playlist, album, artist, Home, Search and Library — is now frosted artwork material: the track's own cover blurred under the lyrics-card veil (`LyricsMaterial`) on an opaque `DhunColors.surface` base plus a soft shadow, i.e. the rule `GlassCard(opaqueBase = true)` exists for, because the menu floats over a dimmed scrim and must never let the page read through its labels. Sized by new `DhunSpacing.menu*` tokens (Material's 280dp ceiling, 220dp floor, 44dp rows, 40dp header thumbnail), no divider, no Close button (tap outside / Back dismisses, unchanged): ≈340dp at the tallest against ≈470dp before. Same actions, order, visibility rules and dismiss-then-navigate ordering, now pure data — `TrackMenuAction` + `TrackMenuPolicy` — pinned by `TrackMenuPolicyTest` (+6). Only the labels lost their parentheticals ("Go to artist (Queen)" → "Go to artist", "Download for offline" → "Download"): the header already names the track and artist, and the long label was what stretched the sheet. The queue row's ⋮ in `PlayerTabs` stays an anchored Material3 `DropdownMenu` with its own actions, but its Material container is switched off (transparent, zero tonal/shadow elevation) and it draws into the same `TrackMenuSurface` / `MenuActionRow` — one menu system, not two. Queue rows themselves are unchanged (they still do not carry the lyrics material; only their menu does).

**CI record (all watched to completion, all green on `80fe28b`).** Push CI **35692075343** · PR CI **35692215784** — all 12 steps, including `:shared:jvmTest` (the new parser and menu-policy cases), the Android Robolectric suite, `assembleDebug`, probe compiles, desktop `compileKotlinJvm` + `jvmTest`, and the packaging/fixture helper tests · Build APK **35692215748** · test-release **35692215751** (`apk` + `msi` success with the hosted install-over; `aab` / `publish` / `release_draft` skipped because a PR cannot publish). The only annotation on the run is the standing `ubuntu-latest` → Ubuntu 26 migration notice (begins 2026-10-19): no Kotlin error annotations, no Node-20 deprecation warning on this head.

**Local verification only — no JDK.** This sandbox has no `java`/`javac`/`kotlinc`/`gradle` and no Maven, Adoptium, Gradle or Google egress, so nothing was compiled locally; CI is the compiler and it is what turned green above. What did run locally: `scripts/validate_fixtures.py` (35 files PASS) and its 29 unittests (OK), plus brace/symbol checks on every edited Kotlin file. All new APIs were verified against the pinned Compose Multiplatform **1.8.2** upstream sources through the GitHub API — the one reachable registry — which is also the trick to remember: material3 `DropdownMenu(shape, containerColor, tonalElevation, shadowElevation)` exists (`border` is newer; not used); the popup column uses `width(IntrinsicSize.Max)`, so a fixed-width child keeps the menu menu-sized instead of stretching to the pane; foundation `clickable(enabled, onClickLabel, role, onClick)` is the pinned signature. Tag refs must be `v1.8.2`, not `1.8.2`. Also confirmed in-repo: `internal` commonMain declarations are visible to `:shared`'s `jvmTest` across packages (precedent `PlayerSheetLayoutTest`), which is what lets a pure policy test pin a dialog living in `ui/components`.

**Docs.** `CHANGELOG.md` carries one entry per defect plus a dated "Verified — PR #114 CI on `80fe28b`" block with the run IDs; `docs/verification/09-browse-pages.md` has a dated section (parser fallback, `PageArtworkBackdrop`, PlaylistScreen left opaque) and updated fixture/test lists; `docs/verification/07-home-search.md` has a dated note on what the menu is now versus the stale Phase-07 list (which still names a favourite row that no longer exists); `.ai/DEBUG_LOG.md`, `.ai/KNOWN_LIMITATIONS.md` and this section carry the root causes, environment traps and limits. No CI identity was written anywhere before it was observed.

**To finish this session.**
1. Merge PR #114 with `gh pr merge --merge` — this repo merges with merge commits, not squash (see PR #113) — then watch the post-merge CI and the rolling `test` republish, and record the new identity (target commit, APK/MSI byte sizes and sha256 digests, published-at) **in a PR comment**. Repo rule: never pre-claim an unpublished identity in memory or docs.
2. Only after that republish is `https://github.com/99ggprooo00-code/DHUN/releases/download/test/dhun-test.apk` / `dhun-test.msi` worth re-downloading; at this writing the rolling release still targets `03a27b1` (published 2026-09-22T04:20:22Z, APK 18,367,219 B, MSI 112,930,816 B).
3. Device gates CI cannot see: album playback really paints artwork end to end (player, mini player, shell blur, lock screen/widget); album- and artist-page body text stays readable over a *bright* cover with the shared dim 0.40 + scrim; the artist parallax seam reads as intended; the menu feels compact in the hand rather than only in a diff.
4. **Carried-over CI defect, observed today and deliberately not fixed in this PR:** the daily `extraction-health` drill did not fire on 2026-09-22. Newest run of any event is still `35561269411` (2026-09-21T04:30:45Z, `414cd79`, `ENVIRONMENT_BLOCKED` as always); cron is `17 4 * * *` UTC; the workflow timing API reports no `last_run_started_at`. PR #111's workflow edit landed on `main` at `7fcadbe` ~02:58 UTC today — between the two fires — so the wedged-registration scenario the previous handoff predicted is what the run list now shows. Agents get 403 on `workflow_dispatch` (a human Run-workflow click is the on-demand path), and the repair cannot be verified until the next 04:17 UTC, so it belongs in its own PR: attempt-5 fresh-file re-registration is the next session's first task. Details in `.ai/KNOWN_LIMITATIONS.md`.
5. Smallest worthwhile follow-ups: give `PlaylistScreen` the same `PageArtworkBackdrop` treatment (two lines, deliberately out of scope here); and note that a track whose real art differs from its album cover still shows the album cover, because the page never carried the per-track URL.

## PREVIOUS STATE — PR #113 glass/scrim CI-green, docs before merge, 2026-09-22, session `arena/01a0c716-dhun`

Superseded later the same day: PR #113 merged as `03a27b1` and the rolling `test`
release was republished against it — APK 18,367,219 B, MSI 112,930,816 B, published
2026-09-22T04:20:22Z. Everything below is unchanged from when it was written.
