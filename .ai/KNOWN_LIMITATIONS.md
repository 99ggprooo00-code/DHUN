...
- **Follow-up fix (PR #112): `download-artifact@v6` still declares `using: node20`.** The post-merge publish run 35681131229 on `main@7fcadbe` re-flagged it; verified at the upstream source (`action.yml` at the v6 tag). Bumped to **v7** (the Node-24 migration release; v8 deliberately not taken — ESM + hash-mismatch-errors-by-default are behavior changes on the release publish path). PR checks could not catch this: the publish job is main-gated. **Lesson: a Node-20 audit is only proven by runs in which each flagged step actually executes** — post-merge publish runs are part of the evidence, not just PR checks.
