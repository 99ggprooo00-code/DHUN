...
## 2026-09-22 (session `arena/01a0c772-dhun`) — album artwork / page backdrops / compact ⋮ menu are CI-green, not device-verified

- **PR #114 code head `80fe28b` is CI-green; the merge and the republish had not happened when this was written.** Push CI **35692075343**, PR CI **35692215784** (all 12 steps), Build APK **35692215748**, test-release **35692215751** (`apk` + `msi` success; `aab` / `publish` / `release_draft` skipped — a PR cannot publish). No local JDK; those runs are the compiler. Only annotation on the run: the standing `ubuntu-latest` → Ubuntu 26 notice (begins 2026-10-19). Log blobs EOF in-sandbox (257 bytes) — annotations API + step conclusions are the readout.
- **The rolling `test` download does not contain these fixes yet.** It targets **`03a27b1`** (PR #113's merge), published **2026-09-22T04:20:22Z**: APK **18,367,219 B** (sha256 `0491a77bb522143a596bea0fe1210d0b33f06e40b2ebd4f6b891f2fb20f4ce6a`), MSI **112,930,816 B** (sha256 `b3732ecc24e3e016c935ca3ef15bd52ea9f7b7e1431fb6682bd145f2cb04192f`). Re-download only after #114 merges and the publish job moves `target_commitish`; do not treat byte size alone as identity.
- **No hardware eyeball of any of the three fixes.** CI cannot see whether album playback really paints artwork end to end (player, mini player, shell blur, lock screen / widget), whether album- and artist-page body text stays readable over a *bright* cover with the shared dim 0.40 + scrim, whether the sharp→blurred seam under the artist parallax hero reads as intended, or whether the 280dp menu with six actions feels compact in the hand rather than in a diff. That is the user's gate after the republish.
- **`PlaylistScreen` still paints an opaque `DhunColors.background`.** Same defect class as the album/artist pages, deliberately left out of scope for this session (the ask named the album page, then the artist page). It is a two-line follow-up: transparent page + `PageArtworkBackdrop(detail.thumbnailUrl)`.
- **Detail pages now paint a second blurred artwork layer over the shell's.** Album/artist backdrops are additional `Modifier.blur` layers composited above the shell's `NowPlayingBackdrop` (which stays composed behind them). This is the pre-existing full-player pattern, and each blur is a static layer rather than per-frame work, but it is one more RenderEffect on screen while a detail page is open — worth watching on low-end Android if the page ever stutters.
- **The album cover is a fallback, not per-track art recovery.** Rows that ship their own thumbnail keep it; rows that ship none inherit the *album* cover. A track whose real art differs from the album cover (a single edit, a deluxe disc) still shows the album cover, because the page never carried the per-track URL. Nothing is invented when a page has neither.
- **The ⋮ menu is a centered dialog, not an anchored popup, by design.** It is mounted once from `DhunAppShell`, which is why one change reaches all seven surfaces; anchoring it to each ⋮ would need anchor plumbing through seven call sites. The queue row's ⋮ *is* anchored (Material3 `DropdownMenu`) and shares the same surface and rows — two mount mechanisms, one menu look.
- **Menu labels no longer name their target.** "Go to artist (Queen)" → "Go to artist"; the menu header carries the track and artist instead. A screen-reader user hears the header and then the rows, so nothing is lost, but the row text alone is less explicit than it was.
- **The daily `extraction-health` drill did not fire on 2026-09-22 — the schedule looks wedged.** The workflow's only automatic trigger is `schedule: cron '17 4 * * *'` UTC. The newest run of any event is still **35561269411**, `2026-09-21T04:30:45Z`, `414cd79`, failed as the drill always does (`ENVIRONMENT_BLOCKED` — no YouTube egress from the runner). Nothing ran or queued at 04:17 UTC today, ~2h before this was checked, and `GET /actions/workflows/extraction-health.yml/timing` returns no `last_run_started_at`. PR #111's workflow edit merged to `main` at `7fcadbe` on 2026-09-22 ~02:58 UTC — after the 09-21 fire and before the 09-22 one — so the first scheduled fire after that edit never happened, which is exactly the wedged-registration scenario the previous handoff predicted. This is **not** caused by anything in PR #114 (no workflow file is touched here) and was **not** fixed here: re-registering means editing the drill workflow, which does not belong in a UI PR and cannot be verified until the next 04:17 UTC. Agents get 403 on `workflow_dispatch`, so the on-demand path is a human Run-workflow click; the repair is the next session's first task (attempt-5 fresh-file re-registration).

- **Carried over unchanged:** the dark-surface ceiling (`#1E1E1E` is 3.12:1, `#222222` 2.97:1 against the WCAG 1.4.11 pin), radio station chain in-memory, S3/S6 hardware items, the #105 checklist, drill `ENVIRONMENT_BLOCKED` steady state, issue #14 user-closable (agent 403 on issue writes), no in-app Android EQ, asset/log blobs EOF, no JDK in the sandbox.


## 2026-09-22 (session `arena/01a0c716-dhun`) — glass / scrim restyle CI-green on the PR head; not on a device, not in the rolling download yet
...
- **Carried over unchanged:** radio station chain in-memory, S3/S6 hardware items, the #105 checklist, drill `ENVIRONMENT_BLOCKED` steady state, issue #14 user-closable (agent 403 on issue writes), no in-app Android EQ, asset/log blobs EOF (annotations API is the readout).
...
- **Drill watch pending at this writing (never pre-claimed):** PR #111 merges BEFORE the 2026-09-22 04:17 UTC cron; the scheduled run on the edited registration is the watch. Expected: fires, classifies `ENVIRONMENT_BLOCKED`, exit 2 (steady state). A NON-FIRING schedule = wedged re-registration → attempt-5 fresh-file re-registration (new path+name) is the next session's first task. In-place-edit safety evidence: this file survived three edits on 2026-09-18 (`7928774`, `19f1e8e`, `a7c4d5d`) with the schedule firing afterwards (09-19/09-20/09-21 runs).
