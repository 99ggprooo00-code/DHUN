...
**Phase/status:** Stage **S1 — restore the maintenance contract**. The live gate ran as workflow **35306224822** on current `main` and returned **RED**. S1 remains open and S2 is blocked. The evidence is mixed: offline/metadata/search/related passed, Home continuation parsing failed, and stream resolution was bot-gated with `AuthRequired`. A narrow tokenless Home parser candidate was CI-verified on `005b526`, but owner run **35308796439** on that candidate still returned Home `Parse` with a top-level `contents` shape. A follow-up direct-contents candidate is now under CI; S1 remains RED.

**Exact files/task currently being worked on:** `shared/src/commonMain/kotlin/dev/dhun/innertube/HomeFeedParser.kt` and `Parsers.kt` now accept shelf-specific Home continuation wrappers/renderers (`musicShelf*`, playlist/carousel variants) and emit only key names in unmatched-shape diagnostics; `HomeFeedParserTest.kt` plus three synthetic fixtures cover the first and follow-up candidates. `tools/playback-probe/.../Main.kt` is unchanged. PR **#91** remains open; the follow-up is under CI and live validation is still pending.

**GitHub state verified live (2026-09-18 04:17 UTC):
- `origin/main` remains `33e94b0`; PR **#91**'s current CI-verified evidence head is `005b526` (full `005b52665331bc415cc2d4c312586fda20c82454`). The session PR is open; PR #54 remains the separate research-only contingency PR.
...
**Last actual error:** live run **35306224822** is RED. The `AuthRequired` results are consistent with GitHub-runner bot-gating, but the separate `home-more` parser failure is not dismissed as network evidence; its raw continuation shape is not yet captured. The first parser candidate is CI-verified but was disproven as sufficient by owner run **35308796439**; its sanitized shape shows a top-level `contents` response. The direct-contents follow-up cannot be called a live fix until CI and another owner-triggered probe validate it.

**Evidence state:** main changes are pushed, merged through PR #90, CI-verified, and released to rolling `test`; PR #91 is open; candidate run **35308796439** is RED with the direct `contents` parser gap. Follow-up head `526e390` is PR-green in **35309128614**, **35309128640**, and **35309128612**; independent push CI **35309124090** flaked on an unrelated `LibraryViewModelTest` timeout, then push CI **35309533562** passed on docs-only successor `98843af`. PR #91 is now `CLEAN`, but the follow-up remains unverified live. The baseline live gate remains RED in run **35306224822** with artifact `rot-drill-35306224822`; residential playback remains unverified, and PR #91 is not merged.

**Exact next technical step and blocker:** have the owner run `extraction-health` against current candidate branch head `98843af` (parser code is unchanged from `526e390`). Confirm that the live response is covered before treating Home as repaired. The agent token cannot dispatch this workflow (HTTP 403). Treat `AuthRequired` as runner-network evidence; do not add cookies, PO tokens, or ADR-007. Do not start S2 or merge PR #91 until the mixed RED result is reconciled.
