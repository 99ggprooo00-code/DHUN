...
**Last error / CI:** none on this head yet — local Gradle remains impossible (no JDK; Maven/Gradle/dl.google.com egress-blocked → `curl` returns `000`). `python3 -m unittest discover -s scripts -p 'test_*.py'` = **27 OK** locally (was 24; +3 `test_ci_workflow`). CI is the Kotlin compiler.
...
1. **Backdrop.** Home, Search and Library now sit on the artwork of the **currently playing** song, blurred 64dp, darkened and edge-to-edge under the content. The source is the playback state the shell already observed for its ambient tint (`PlayerViewModel.currentTrack`) — there is no second "now playing" state, so a track change crossfades the backdrop (Coil `crossfade`) on whichever screen is showing, and what a screen *lists* never becomes the background. It requests the **544px list tier**, not the 1024px now-playing tier, and nothing animates per frame. **Fallback is the old background, drawn by absence:** no thumbnail / blank URL / failed load / a platform that cannot really blur → the composable draws *nothing*, so no glyph, grey rectangle or spinner is ever stretched across a screen.
2. **Android BACK.** Search and Library are **tabs**, not `detailStack` routes, so `BackHandler { if (!nav.closeTop()) moveTaskToBack(true) }` found nothing to close and parked the app from a screen in use. `AppNavState` now records tab switches (in the `selectedTab` setter, so nav bar, rail, "Liked songs"/"Offline" and launcher-shortcut assignments all count) and `onBack()` runs the full contract: player → one page → one tab → platform default. `AppTab.root` = HOME keeps the old root behaviour and makes a loop impossible; the history rides the existing Bundle, so rotation/process death does not drop the user to Home.
3. **Windows rails.** Root cause read out of the pinned Compose Multiplatform **1.8.2** sources, not assumed: `Modifier.scrollable` gates its drag detector on `CanDragCalculation = { it.type != PointerType.Mouse }` (`Scrollable.kt`, beside the upstream TODO *"provide public way to drag by mouse"*) and the desktop mediator reports every mouse event as `Mouse`, so hold-and-slide never worked; separately `canConsumeDelta` (`MouseWheelScrollable.kt`) projects a wheel delta onto the container's own axis, so a vertical wheel over a rail is declined and the page scrolls — **that is correct and was left alone: the wheel stays a vertical control.** Trackpad two-finger pans already worked (Windows `WM_MOUSEHWHEEL` → AWT sets `SHIFT_DOWN_MASK` → mediator emits an **x** delta) and keep their native path and native smooth animation, because nothing added here observes `PointerEventType.Scroll`. One shared component — `DhunHorizontalRail` — now backs **all 12 rails** (Home ×4, Search ×4, Artist ×4) and adds mouse hold-and-slide plus a glassy draggable scrollbar that appears only when content overflows; the accent chip row and the player's transport row get the same drag via `Modifier.dhunMouseDragScroll`. No global horizontal scroll anywhere.

**Files:** `shared/…/design/components/HorizontalRail.kt` (new) · `shared/…/design/components/NowPlayingBackdrop.kt` (new) · `shared/…/design/BlurSupport.kt` + `androidMain`/`jvmMain` actuals (new) · `shared/…/design/DhunSpacing.kt` (`railScrollbar`) · `shared/…/ui/shell/{AppNavState,DhunShellLayout,DhunAppShell}.kt` · `shared/…/ui/{home/HomeScreen,search/SearchScreen,browse/ArtistScreen,player/FullPlayer}.kt` · `shared/…/design/components/AppearanceControls.kt` · `app-android/…/{MainActivity.kt,ui/NavStatePersistence.kt}` · tests: `HorizontalRailTest`, `NowPlayingBackdropPolicyTest`, +8 `AppNavStateTest`, +2 `DhunShellLayoutTest`, +2 `NavStatePersistenceTest` · `CHANGELOG.md` · `.ai/KNOWN_LIMITATIONS.md` · this file.

**Last error / CI:** local Gradle is still impossible (no JDK; Maven/Gradle/dl.google.com egress-blocked → `curl` returns `000`), so CI is the compiler, and it caught two real mistakes that were fixed here rather than merged: (1) `:shared:compileKotlinJvm` red at `HorizontalRail.kt:183,189` — inside the gesture callbacks `launch` bound to the deprecated scope-less `kotlinx.coroutines` global, because in CMP 1.8.2 `PointerInputScope`/`AwaitPointerEventScope` are `Density`, **not** `CoroutineScope` (verified against the shipped `ui.api`/`foundation.api` dumps) → both drag paths now use `ScrollableState.dispatchRawDelta`, which is synchronous and also drops a coroutine launch per drag event; (2) `:app-android:compileDebugUnitTestKotlin` red at `NavStatePersistenceTest.kt:122` — JUnit's `assertFalse` takes the message **first**, unlike `kotlin.test`. **CI is green on the head** (`build-and-test` 6m0s, `apk`, `msi`, `build` all pass) and the green is *proven meaningful* by two mutation runs: shrinking `RailScrollbarGeometry.MIN_THUMB_FRACTION` 0.18f→0.05f went red on `HorizontalRailTest.thumbFractionIsFlooredSoItStaysGrabable(HorizontalRailTest.kt:49)`, and reversing the expected history order went red on `NavStatePersistenceTest.tab back history survives a round trip…(NavStatePersistenceTest.kt:111)`. That second mutation also settles an environment question the docs had wrong: **`:app-android:assembleDebug` executes the app-android unit suite, it does not merely compile it.** Both mutations are reverted; `MIN_THUMB_FRACTION` is back to `0.18f`.
...
<details>
<summary><b>Prior snapshot (`arena/01a0a7f3-dhun` — PR #67 Full Player ⇄ Related/Queue sheet transition; no longer the active task, kept verbatim)</b></summary>
...
<details>
<summary><b>Prior snapshot (`arena/01a08976-dhun` — PR #56 repair + #58/#59 marks; `main` GREEN)</b></summary>

Updated **2026-09-10 (UTC, post-merge)** · session **`arena/01a08976-dhun`** (repair-`main`) · **DONE: `main` is GREEN again.** `origin/main` = **`be51d7d`** = **PR #56 MERGED** at `2026-09-10T04:21:16Z`; the repair was verified on GitHub before and after the merge — `build-and-test` **pass** (run `465`, `34436859375`, all steps incl. `:shared:jvmTest` and `Android debug build` = `:app-android:assembleDebug` with its coupled unit suite), `apk` + `msi` + **`publish`** **pass** (test-release `183`), and `Build APK` **pass** (run `6`) — the first green run that workflow ever had (`1` and `2` were both red). Rolling **`test`** republished `2026-09-10T04:25:25Z`: `dhun-test.apk` 17,840,191 B, `dhun-test.msi` 112,762,880 B, both + `.sha256`. Marks landed via **PR #58 → `7d621f8`** (the only delta since `be51d7d` is docs — `main` is now `7d621f8`+), and the ROADMAP lags `main` by exactly that one docs commit, as the ritual says it must. Below is the record of what was broken and how it was proven — it is kept verbatim because the *diagnosis* (not the repair) is what the next session needs. `main` **was** RED at `58d9ac8`: `build-and-test`, `apk` and `msi` fail on every push to `main` since `073083c`, and the brand-new `Build APK` workflow has never been green. Everything here is read **off GitHub** (`gh api …/check-runs/…/annotations`, `gh pr checks 55`, `gh api …/commits?per_page=12`, `gh api …/actions/workflows/*/runs?branch=main`), not from the sandbox: this clone is shallow at `58d9ac8`, working tree clean, zero local-only commits, and **there is no JDK in this sandbox — CI is the only compiler.**
...
**Status of the steps below — all executed:** branch pushed → PR #56 → gates green → merged `be51d7d` → `main` CI green → `test` republished. Two findings CI produced that are worth keeping: `:shared:jvmTest` and `:app-android:testDebugUnitTest` **ran and passed** on the first green head since `cd97464`, so the compile failure had indeed masked nothing worse than itself; and the Python step ran `scripts/test_apk_workflow.py` green, which had already been proven red against the old `build-apk.yml`.

**Cross-session note (verified, not assumed):** `arena/01a0897a-dhun` / **PR #57** was repairing the same two errors in the same file while #56 was in review (`gh api …/compare/070497d...f38ee83`: `InnerTubeClient.kt +137/−15`, `OwnClientStreamResolver.kt +72/−18`, new `AltPlayerIdentityTest.kt +281`), and it goes further — it *sources* visitor data + signature timestamp and races them through every wave, i.e. it owns the half #56 deliberately refused to fake. Its `build-and-test`/`apk` were green at `f38ee83`; its `Build APK` job was **red only because its base predates the `build-apk.yml` fix** — rebasing onto `be51d7d` inherits it. **It also asserts a different wire shape than the merged code** (`9aaccd0` "Fix playbackContext placement: top-level per InnerTube schema, not nested in context"), which CI cannot arbitrate and which no live capture has decided; recorded as an open question, not a settled fact.

**Exact next step (for whoever picks this up):** **(0) Restore the rot drill — it has not
run since `#10` @ `2026-09-07T04:28Z`** (three daily slots missing, workflow
`state=active`, agents get **403** on `workflow_dispatch`, so this is a human click: *Run
workflow* on `main` + a look at Settings → Actions). Until then the Doctrine's "breakage
detected within 24 h" is not being held, and the red `rot-drill` entries in the run list are
**0-job no-trigger noise, not verdicts** — read the whole rot-drill section before
re-citing any of them. **(1)** #57 rebases onto `be51d7d` and re-runs; if its `playbackContext` placement changes the request shape, that needs a **live** probe verdict (or a captured YouTube request) as evidence, not a compile — green CI proves it *builds*, never that YouTube accepts it. (2) **ADR-007 needs the user's verdict** (open PR #54, PROPOSED) before any attestation/token path is implemented; issue #14 stays open meanwhile. (3) Board hygiene from the stranded-branch audit run this turn (`gh api …/compare/main…<ref>`): `arena/01a085ea-dhun` and `fix/android-player-visitordata` are **fully merged (ahead 0)**; `arena/01a08455-dhun` (3 stale docs commits) and `arena/01a08676-dhun`/**PR #53** (3 commits, its `.ai/ROADMAP.md` hunk is `+17/−513` — **do not merge as-is**, it would delete this file) carry nothing that #54's research does not already cover. (4) `rot-drill` remains red — the push-triggered run on `be51d7d` (`110`) produced no preserved verdict and no issue #14 comment, so it neither confirms nor denies extraction progress; the next scheduled drill (`17 4 * * *`) is the measurement, and it now at least measures the right build — and read the *whole* CI log, because a compile failure masks the suites behind it: `:shared:jvmTest` and `:app-android:testDebugUnitTest` have not executed on `main` since `cd97464`; (2) `gh pr merge --merge` only when green — `main` red freezes the rolling `test` build at `cd97464` (published `2026-09-10T02:02:44Z`) and blocks every parallel session; (3) then the actual root-fix decision, **ADR-007** (draft on open PR #54, status PROPOSED — a locked-decision change, so it needs the user's OK and its own gates) rather than another protocol guess. Board check: **PR #53 must NOT be merged as-is** — its `.ai/ROADMAP.md` diff is `+17/−513`, i.e. it would delete nearly this whole file from a stale base. `rot-drill` red on `main` (`34434063405`… run `106`) is pre-existing issue #14 (GitHub-runner IP gating) and is not one of the three required gates.
...
**⚠️ Standing correction, found while verifying the repair (2026-09-10).**
`rot-drill` has **no `push`/`pull_request` trigger** (`on: schedule` +
`workflow_dispatch`), so every "red rot-drill" run Actions shows on a push —
`104`–`113` today, including on `main` — is a **0-job, no-matching-trigger
failure**, not a probe verdict. Check `…/actions/runs/<id>/jobs` →
`total_count: 0` before ever citing one. **This is why several ROADMAP
snapshots read "rot-drill red everywhere = issue #14": that evidence was
mostly noise.** The real signal is `event=schedule`: **#10 @ `d1e0408`
(`2026-09-07T04:28Z`) failed** (and `#4`/`#3`/`#2` on 09-03…09-05 were
**green**), and — the actual new defect — **no scheduled run has fired since
2026-09-07**, three daily slots missing, while the workflow reports
`state=active`. Agents cannot dispatch it (`gh workflow run` → **HTTP 403**,
re-verified this session), so a human must press *Run workflow* on `main` and
Settings → Actions needs a look. Until then Phase 14 acceptance criterion 1
("rot drill green **and** scheduled") is **regressing, silently** — which is
exactly the failure mode the drill exists to prevent.

**GitHub snapshot, 2026-09-10 (post-PR-#56), read off GitHub (`gh api`,
`gh pr checks`, `gh release view`) — not from this clone, which is shallow:**
`main` = **`be51d7d`** and it is **GREEN** — `build-and-test` `34436859375`
success, test-release `183` `apk`/`msi`/`publish` success, `Build APK` run `6`
success. The outage this closed: from `073083c` (PR #55, merged with its own
required gates **red**) through `58d9ac8`, `build-and-test` `456`–`458`,
`apk`+`msi` `177`–`179`, `Build APK` `1`–`2` and `rot-drill` `104`–`106` all
failed; `:shared:compileKotlinJvm` was the cause, so the suites behind it
never ran either. Repair + evidence are in **CURRENT ACTIVE TASK** above.
...
| Daily rot-drill workflow + failure alerts | 🔴 **Mechanism ✅, cadence BROKEN.** The workflow is merged and its schedule+alert path was proven by real `event=schedule` runs (`34011539225` on `dd1ab31` failed → uploaded the artifact → auto-commented on issue #14 at `04:29:14Z`); the last such run is **#10 on `2026-09-07T04:28Z`**, and **#2/#3/#4 (09-03…09-05) were green**. **Nothing has fired since** (`state=active`, three slots missing), and the red `push`-event entries in between are 0-job noise, not verdicts — see the correction above. | 🟨 A green live production-probe verdict and auto-close-on-recovery remain unproven; placeholder-workflow greens do not count · **Streak verified 2026-09-10:** runs `105` (`main@6e4d057`) and `106` (`main@58d9ac8`) red, as are `101`–`104` — but those four ran on a `main` that no longer compiles, so a probe verdict there is meaningless; the first drill after the repair is what actually measures extraction |
...
| **`main` build health** (the precondition every other row depends on) | ✅ **MERGED — `be51d7d` (PR #56, `2026-09-10T04:21:16Z`).** `main` had been uncompilable since `073083c` (PR #55 merged with `build-and-test`/`apk`/`msi` red at its own head): `InnerTubeClient.kt:268` (`put(key){…}` inside `buildJsonObject` → needs `putJsonObject`) and `:300` (`visitorData` read in `postAltJson`, which never received the parameter). Plus `Build APK` failing on `:app:assembleDebug` — no such module ("project 'app' is ambiguous… Candidates are: 'app-android', 'app-desktop'"). Repaired with both fixes, an `X-Goog-Visitor-Id` null-guard (empty was worse than absent), 2 `MockEngine` wire-shape tests, and `scripts/test_apk_workflow.py` (module/artifact-path contract, proven red-then-green). **CI on `be51d7d`: `build-and-test` `34436859375` success · `apk`/`msi`/`publish` run `183` success · `Build APK` run `6` success (its first ever).** `:shared:jvmTest` + `:app-android:testDebugUnitTest` executed green — the masking is gone | 🔴 **Still open: nothing here fixes gated playback.** #55's session knobs remain inert until ADR-007 lands (PR #57 is the attempt); audible Android playback on the user's network is the gate, and it is not claimed |
