...
**Last error / CI:** local Gradle remains impossible (no JDK; Maven/Gradle/dl.google.com egress-blocked → `curl` returns `000`), so CI is the compiler: `python3 -m unittest discover -s scripts -p 'test_*.py'` = **24 OK** locally; CI `35048954505` on `c0b27fc` covers `:shared:jvmTest`, `:app-android:assembleDebug`, `:app-desktop:compileKotlinJvm` and the probe compile — verdict recorded in the follow-up commit + PR body. `rot-drill` 0-job noise is not a gate (see the snapshot below).
