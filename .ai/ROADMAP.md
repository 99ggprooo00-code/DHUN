...
**Exact next action / session boundary:** automatic branch CI has passed all checks at `77f9c96`; the remaining blocker is manual workflow dispatch permission. Ask the owner to reconnect GitHub in Arena or use the minimal GitHub Actions step below. Do not retry or bypass the 403 without that change. The manually selected workflow ref must be **`arena/01a0759b-dhun`**, with `build_only=true` (this branch defaults it on and cannot publish even if it is off). Once dispatch access or an owner-started run is available, monitor actual APK/MSI builds, property/sentinel checks, artifact provenance and `publish=skipped`; do not claim an MSI exists before then. Keep this session and branch; no PR/merge, release, branch cleanup or one-time finalization action.
...
**Owner's minimal unblock:** GitHub → Actions → `test-release` → **Run workflow** → choose **`arena/01a0759b-dhun` (NOT main)** → keep **build_only** enabled if shown → Run workflow. The selected branch defaults build-only on and its publish job cannot run on non-main. This is a repeatable GitHub build action, not an Arena PR/finalization action. Once a run exists, monitor it and inspect real package/sentinel/artifact results before recommending installation.

**Commit/push status:** all automation and tests are pushed; this result record is a documentation-only follow-up on the same branch. Do not treat parsing PowerShell as executing an MSI, and do not create/merge a PR, release or extra branch to get around the dispatch blocker.
