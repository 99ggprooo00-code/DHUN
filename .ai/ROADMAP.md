...
**Exact next action:** finish the final automation review in `.github/workflows/test-release.yml` and `scripts/test_build_workflow.py`, run local helper tests, commit/push this update plus the permission/evidence docs, and verify branch CI. Open the authorised PR from this session branch to main. Installer packaging and the disposable-Windows install-over check are now ordinary **pull-request checks**, with read-only build permissions and `publish` forced off for PR refs; verify those real jobs before merging, rather than treating PowerShell parsing as installer proof. Manual workflow-dispatch permission remains denied and will not be retried. Fix actual failures on this same branch, then merge only after green checks. Verify automatic main CI/test publishing afterward and leave all user-machine/audio/visual gates open.
...
**Commit/push status:** application and helper work is already pushed/CI-green through `455743b`. The current local change adds pre-merge package checks and updates this instruction record. No PR is open yet and no MSI/native smoke result exists; record actual run evidence before changing those marks.
