...
**Evidence state:** main changes are pushed, merged through PR #90, CI-verified, and released to rolling `test`; PR #91 is open; the first parser candidate at evidence head `005b526` is CI-green, but candidate run **35308796439** is RED with the direct `contents` parser gap. The follow-up is unverified. The baseline live gate remains RED in run **35306224822** with artifact `rot-drill-35306224822`; residential playback remains unverified, and PR #91 is not merged.

**Exact next technical step and blocker:** run CI on the direct-contents follow-up, then have the owner run `extraction-health` against the updated candidate branch. Confirm that the live response is covered before treating Home as repaired. Treat `AuthRequired` as runner-network evidence; do not add cookies, PO tokens, or ADR-007. Do not start S2 or merge PR #91 until the mixed RED result is reconciled.
