...
**Last actual error:** live run **35306224822** is RED. The `AuthRequired` results are consistent with GitHub-runner bot-gating, but the separate `home-more` parser failure is not dismissed as network evidence; its raw continuation shape is not yet captured. The parser candidate is now CI-verified but cannot be called a live fix until a sanitized owner-triggered probe validates that response shape.

**Evidence state:** main changes are pushed, merged through PR #90, CI-verified, and released to rolling `test`; PR #91 is open; the parser candidate at evidence head `005b526` is CI-green in runs **35307916827**, **35307913766**, **35307916736**, and **35307916668**. The current live gate is still RED in run **35306224822** with artifact `rot-drill-35306224822`; live Home and residential playback remain unverified, and PR #91 is not merged.

**Exact next technical step and blocker:** have the owner run `extraction-health` against the CI-green candidate branch or provide a sanitized Home continuation shape. Confirm that the live response is covered before treating Home as repaired. Treat `AuthRequired` as runner-network evidence; do not add cookies, PO tokens, or ADR-007. Do not start S2 or merge PR #91 until the mixed RED result is reconciled.
