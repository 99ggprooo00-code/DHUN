...
Updated **2026-09-06 (UTC)** · session **`arena/01a0759b-dhun`** · last verified GitHub `main` / `test` **`0920148`** · rolling release published **`2026-09-06T07:22:29Z`**. The repairs below are a **CI-green branch candidate, not in that installer**. Latest verified implementation HEAD **`77f9c96`**, [CI **34028225356 success**](https://github.com/99ggprooo00-code/DHUN/actions/runs/34028225356). Build-only packaging is prepared but manual dispatch is blocked by integration permissions.

**Latest user instruction — PR and merge authorised:** “Ok complete this work then PR and merge.” This supersedes the prior no-PR/no-merge checkpoint restriction for this repair batch. Continue exclusively on `arena/01a0759b-dhun`, preserve it after merge, and do not use unrelated one-time Arena finalization controls or delete old branches. Complete review and real automated checks, then open/merge the PR using GitHub. This is not approval to mark playback/visual/hardware gates passed or tag v0.1.0.
...
**Current technical task — branch artifacts, no session finalization:** `.github/workflows/test-release.yml` now has a default-on `build_only` dispatch input, read-only build permissions and a job-level main/ref/mode publishing guard. Non-main refs cannot publish even if that input is false, and branch builds cannot cancel main's release group. Reusing this workflow preserves the MSI run-number sequence. `scripts/stage_artifact.py`, `scripts/msi_helpers.ps1`, `scripts/stage_msi.ps1` and `scripts/check_msi_upgrade.ps1` add exact binary provenance, actual MSI property validation and a disposable-hosted-Windows install-over/userdata/uninstall smoke check. `docs/verification/windows-candidate.md` goes in the artifact for the user. Python publishing/provenance tests and PowerShell syntax now **pass in CI 34028225356**; actual MSI property/install execution is still unverified. Do not call the Windows candidate built yet.

**Current blocking error — do not merge PR #30 yet:** PR packaging run **34029598179** built MSI **1.33.1** and verified its identity, then the real hosted-Windows test failed: **“In-place MSI upgrade removed/changed existing userdata.”** Its MSI artifact was withheld and `publish` skipped. PR CI 34029598196 passed, but compilation is not sufficient. Source review confirms jpackage's versioned HKCU cleanup registration and unconditional `RemoveFolderEx` for INSTALLDIR during old-product removal.

**Exact files now being fixed:** `scripts/patch_msi_upgrade.ps1` finalizes the unsigned MSI before hashing; `scripts/prepare_legacy_msi_upgrade.ps1` bridges old packages by suppressing only DHUN's matching HKCU recursive-cleanup value before removal (it never moves/deletes userdata). New packages clear their cleaner property only when `UPGRADINGPRODUCTCODE` is set and retain explicit-uninstall cleanup. `scripts/stage_msi.ps1` applies this policy; `scripts/check_msi_upgrade.ps1` now tests legacy install-over, upgrade-flag removal, reinstall and explicit uninstall. These changes require real Windows re-verification; 21 Python helper tests pass locally but do not prove MSI execution.
...
**Exact next action:** push the installer data-safety correction and failure evidence to the existing PR #30, inspect both code CI and native Windows package checks, and fix any real failures without weakening the sentinels. Only merge the authorised PR after all checks pass. Then inspect automatic main CI and rolling test publishing, preserving this session branch. No stable v0.1.0 or successful playback/visual acceptance can be claimed from installer tests.
...
**Current GitHub evidence:** `6fedf8a` / `9317050` added build-only packaging and its guide; CI 34028039448 passed on `9317050`. Follow-up **`77f9c96`** added PowerShell parsing; [CI **34028225356**](https://github.com/99ggprooo00-code/DHUN/actions/runs/34028225356) **SUCCESS**, job 101472922356 finished **10:46:45Z, 2026-09-06**. Python helper tests, PowerShell AST syntax, shared JVM tests, Android debug build and probe/Desktop compilation all passed; check annotations: 0. The packaging workflow's branch run count is **0** after the denied dispatch. Main/test still point to `0920148`, release 07:22:29Z, no PRs.

**Dispatch versus PR checks:** the earlier manual API dispatch was denied (403); no credential change or retry is being attempted. The user has now explicitly requested a PR/merge, and Windows packaging is being added to the normal PR verification path. PR refs can never run the publish job. Main pushes keep the existing rolling-test policy and MSI counter; no v0.1.0 release is planned.

**Commit/push status:** application and helper work is already pushed/CI-green through `455743b`. The current local change adds pre-merge package checks and updates this instruction record. PR #30 is OPEN and unmerged. MSI 1.33.1 built but failed userdata preservation; record the corrected Windows run before any merge.
