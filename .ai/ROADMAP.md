...
**Evidence state:** main changes are pushed, merged through PR #90, CI-verified, and released to rolling `test`; PR #91 is open; candidate run **35308796439** is RED with the direct `contents` parser gap. Follow-up head `526e390` is PR-green in **35309128614**, **35309128640**, and **35309128612**; independent push CI **35309124090** flaked on an unrelated `LibraryViewModelTest` timeout. The follow-up remains unverified live. The baseline live gate remains RED in run **35306224822** with artifact `rot-drill-35306224822`; residential playback remains unverified, and PR #91 is not merged.

**Exact next technical step and blocker:** settle the unrelated push-test flake with a green push check, then have the owner run `extraction-health` against the updated candidate branch `526e390` (or its docs-only successor). Confirm that the live response is covered before treating Home as repaired. Treat `AuthRequired` as runner-network evidence; do not add cookies, PO tokens, or ADR-007. Do not start S2 or merge PR #91 until the mixed RED result is reconciled.
