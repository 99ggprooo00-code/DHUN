...
**Last error / current fix:** branch CI [34025629231](https://github.com/99ggprooo00-code/DHUN/actions/runs/34025629231) at **`f914050` FAILED** during `:shared:compileKotlinJvm`: four **`Unresolved reference 'index_'`** errors in `shared/src/commonMain/kotlin/dev/dhun/ui/home/HomeScreen.kt` (lines 393, 407, 420, 439). Bracing `${index}` fixes the interpolation. The same review found the Quick-picks visibility helper was not connected to the screen's category filter; `remainingHomeSections` now wires it in, with an assertion in `HomePaginationTest.kt`. CI checkout moves to the verified Node-24 `actions/checkout@v5` to remove the reported Node-20 action warning. **Exact next step:** commit/push these fixes to the same branch and rerun CI; Kotlin tests and later build steps have not passed yet.

**Environment distinction:** GitHub installed JDK 17 and reached the Kotlin compiler, so the absent local JDK is no longer preventing verification through the authorised CI path. Local Gradle still cannot run. The CI Python step passed; local checks also pass (10 Python tests, 29 JSON fixtures, `git diff --check`). No new playback/Windows/UI proof exists.
...
**GitHub branch evidence:** four repair/documentation commits were pushed through **`f914050`** to `arena/01a0759b-dhun`. CI **34025629231** failed at shared Kotlin compilation as above; later tests/build steps were skipped. Main/test remain **`0920148`**, release published **07:22:29Z**, no open PRs. New fixes are not counted as verified until their own CI run passes.

**Commit/push status:** approved branch push completed. The follow-up fixes and this evidence update are being prepared for a same-branch push. No PR, merge, tag or release operation has been performed or authorised.
...
The repair batch is now on the session branch, but CI is failing; it is not counted as a completed milestone.
...
| Install/uninstall hygiene | ✅ PR #19 per-user MSI and `DhunUserDirs`, PR #22 startup ruggedization and Android private-storage policy on main; latest MSI rebuilt/published green in `34018809913` | 🟨 Fresh in-place upgrade FAILED with “Another version…”; only manual uninstall/reinstall succeeded. Dynamic MSI versions are a branch candidate (CI failing); upgrade/data preservation and clean-target uninstall still need evidence |
