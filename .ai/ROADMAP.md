...
**Exact next action / session boundary:** commit/push all pending workflow/helper/guide changes plus the saved session clarification to `arena/01a0759b-dhun`. Run branch CI and dispatch **only the artifact build** with `build_only=true` on this same ref. Verify the exact run SHA, both APK/MSI artifact records and checksums, actual MSI ProductVersion/UpgradeCode and the hosted Windows smoke results. Require `publish` to be **skipped**. Fix genuine failures, then provide the Windows artifact/run link and quick test guide. Do not create/merge a PR, mutate main/test, delete branches, or use any one-time session-ending action. No live rot-drill or stable release is part of this task.
...
**Commit/push status:** application/regression work remains pushed and CI-green through `33cc71b`. New packaging automation, provenance tests, Windows test guide and this session-policy clarification are pending the next same-branch push. Local checks now pass **19 Python tests** plus **29 JSON fixture syntax checks** and `git diff --check`; PowerShell/MSI execution is pending on Windows Actions. The current public installer is still the old 07:22:29Z build.
