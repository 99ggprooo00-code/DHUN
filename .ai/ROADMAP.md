...
**Last actual error:** live run **35306224822** is RED. The `AuthRequired` results are consistent with GitHub-runner bot-gating, but the separate `home-more` parser failure is not dismissed as network evidence; its raw continuation shape is not yet captured. The parser candidate is source/test-backed but cannot be called a live fix until CI and a sanitized owner-triggered probe validate it.

**Evidence state:** main changes are pushed, merged through PR #90, CI-verified, and released to rolling `test`; PR #91 is open and its prior docs checks are green. The current live gate is RED in run **35306224822** with artifact `rot-drill-35306224822`; the new parser candidate is unverified, no hardware verification is claimed, and PR #91 is not merged.

**Exact next technical step and blocker:** run CI on the narrow parser candidate, then have the owner run the probe against the candidate branch or otherwise provide a sanitized Home continuation shape. Confirm that the live response is covered before treating Home as repaired. Treat `AuthRequired` as runner-network evidence; do not add cookies, PO tokens, or ADR-007. Do not start S2 or merge PR #91 until the mixed RED result is reconciled.
...
| PR #91 `docs(s1): reconcile extraction-health handoff at main 33e94b0` | OPEN, session branch `arena/01a0b224-dhun`, last CI-verified evidence head `7c043bf` | Docs-only S1 handoff; CI green in runs 35306525058 / 35306521396, Build APK 35306525028, test-release 35306525101. Do not merge without explicit user instruction. |
