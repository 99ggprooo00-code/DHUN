...
**Exact next action / authorisation boundary:** the approved **CI-only code checkpoint is complete** at `75c4a8b`. Record this verified result in `.ai/ROADMAP.md`, `.ai/KNOWN_LIMITATIONS.md`, `.ai/DEBUG_LOG.md`, `docs/verification/12-desktop-native.md`, `docs/verification/14-release.md`, README and CHANGELOG; push the documentation-only follow-up to the same branch and check its CI before reporting. The completed run above validates the code snapshot; the docs-only follow-up does not change build/test inputs.

**After this checkpoint, stop before product publication.** No PR, merge, tag, release, test-release dispatch or rot-drill dispatch is authorised. The next product step requires a separate instruction to arrange a candidate Windows build, then in-place upgrade/data preservation, uncached audible playback, Home scrolling/retry/refresh and visual re-test. Cache/recovery, clean-target checks, soaks and live probe bytes remain open. ADR-003 remains proposed; no identity fan-out.
...
**Commit/push status:** all implementation and regression work is committed/pushed. This final evidence update is documentation-only and goes to the same branch under the existing CI permission. Do not mislabel it as a new app release or close hardware gates because CI passed.
