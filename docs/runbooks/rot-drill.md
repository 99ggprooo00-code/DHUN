...
## State (2026-09-20): drill fires daily; ENVIRONMENT_BLOCKED is the expected runner result — S1 is CLOSED GREEN

- **Current `main`:** `d99060ecff08096359dde4fe590899d5088227b6` (PR #96 merge, 2026-09-20T16:40Z — docs-only, on top of PR #94 `7304abb`, PR #93 `fabeb5f`, PR #92 `39b8748`, PR #91 `6f7fa48`); the release baseline this runbook documents. Post-merge push CI green on that SHA: CI 35523474108 · Build APK 35523474088 · test-release 35523474032. Rolling `test` release targets exactly `d99060e` (published 2026-09-20T16:46:20Z; `dhun-test.apk` 17,948,508 B, `dhun-test.msi` 112,861,184 B + both `.sha256` sidecars — sizes unchanged by docs-only merges). **This is the build the user playback-tested for S1.**
...
- **Gate: S1 is CLOSED GREEN (2026-09-20) and S2 is unblocked.** Residential evidence on `main@d99060e` — home WiFi, no VPN, Android **and** Windows, **4 songs audible with advancing position, zero failures**, Android audio continued on the locked screen. Because `compare/6f7fa48...d99060e` is **documentation-only (0 source files)**, the drill and the user exercised the *same* extraction code, so `ENVIRONMENT_BLOCKED` is datacenter IP gating and trigger **T1 is disproven**.
- **How to read this drill from now on:** `ENVIRONMENT_BLOCKED` on the runner is the **expected steady state**, not an alarm — do not open extraction work off it. Escalate only on (a) a `FAIL` classification, which means a parser/contract break the runner *can* see, or (b) a *residential* failure report from a device. The first scheduled run on `main@d99060e` is due 2026-09-21 04:17 UTC (observed starts land ~04:28–04:30 UTC).
- **Sandbox limits:** raw GitHub logs return `EOF`, the agent cannot dispatch the workflow (HTTP 403) or write issues, and no local Gradle test runs because no JDK is installed.
