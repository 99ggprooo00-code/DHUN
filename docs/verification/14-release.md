...
> PR **#91**'s docs-only S1 evidence head is `7c043bf`; CI **35306525058**, Build APK **35306525028**, and test-release **35306525101** pass (push CI **35306521396**). The PR remains open and unmerged; this follow-up status commit is docs-only. The agent cannot dispatch workflows (HTTP 403), but the owner-triggered run is authoritative. S1 remains open: capture/reconcile the Home continuation shape and perform approved residential/device verification; do not add credentials, cookies, PO tokens, BotGuard, attestation, or ADR-007. No v0.1.0 tag/release or hardware acceptance is claimed.
