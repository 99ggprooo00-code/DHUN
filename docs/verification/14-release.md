...
- [ ] **Home parser candidate pending live validation:** supports `musicShelfContinuation` / `musicPlaylistShelfContinuation` and corresponding action renderers, with synthetic coverage. Do not mark `home-more` green until a candidate-branch probe or sanitized response confirms the live shape.
