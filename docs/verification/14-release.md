...
> PR #91 is OPEN, unmerged, and `CLEAN` at final pushed head **`ad129be`** (code head `6dd98fb` plus a docs-only sync). Push CI **35314651766**, PR CI **35314654589**, Build APK **35314654604**, and test-release **35314654669** all pass on the final pushed head, including shared, Android, playback-probe, extraction-health classifier, Desktop JVM, and packaging checks. Android and Windows/Desktop production fixes remain intact and are not the remaining issue.
> The latest owner rerun is attempt **5** of workflow run **35310771629**, job **105507779324**, still on older head `dbb3c08`, and completed **failure**. Version/search, first Home page, related, and deterministic offline checks passed; `home-more` remained **RED** with `shape=top[contents,responseContext,trackingParams];continuation[-];contents[singleColumnBrowseResultsRenderer];...`. Own-client/yt-dlp remained runner-bot-gated with `AuthRequired`; NewPipe reported `Parse(detail=JSON response is too short)`. No live audio bytes were validated, and the new classifier was not present in this old-head rerun.
> The parser/probe/workflow changes now emit and classify `PASS`, `FAIL`, `ENVIRONMENT_BLOCKED`, or `UNAVAILABLE`; all non-PASS statuses remain non-zero, and only `FAIL` opens a rot-drill issue. The final candidate is not live-accepted. The next gate is an owner-triggered run on `arena/01a0b224-dhun@ad129be`; no S2, hardware, stable-release, or merge acceptance is claimed.
...
| Daily live extraction-health | 🔴 Latest real candidate run **35310771629** tested older PR head `dbb3c08`: offline/metadata/search/related passed; `home-more` had a separate continuation parse failure; own-client/yt-dlp were runner bot-gated; NewPipe was a short-JSON parse watch. Final pushed head `ad129be` (code head `6dd98fb`) has not had a live run. | S1 remains open. Owner must run the final pushed head, then accept `home-more` only on a live pass and keep any `ENVIRONMENT_BLOCKED`/`UNAVAILABLE` resolver result separate. Do not begin S2. |
...
- [ ] **Final candidate validation pending — branch `arena/01a0b224-dhun@ad129be`:** PR #91 is CI-green and `CLEAN` (push CI **35314651766**, PR CI **35314654589**, Build APK **35314654604**, test-release **35314654669**), but no owner-triggered `extraction-health` run has tested this final pushed head. Android and Windows/Desktop production paths are not being reopened. S1 remains RED until `home-more` passes live.
...
- [ ] **Home parser follow-up pending live validation:** the final pushed candidate at `ad129be` (code changes are `6dd98fb`) contains the scoped direct/nested-browse parser support and classifier changes, all CI-green. Do not mark `home-more` green until an owner-triggered probe on final pushed head `ad129be` passes; the older run `35310771629` is evidence only for `dbb3c08`.
