...
> PR #91 is OPEN, unmerged, and `CLEAN` at final head **`6dd98fb`**. Push CI **35313596684**, PR CI **35313601849**, Build APK **35313601854**, and test-release **35313601903** all pass on that exact head, including shared, Android, playback-probe, extraction-health classifier, Desktop JVM, and packaging checks. Android and Windows/Desktop production fixes remain intact and are not the remaining issue.
...
| Daily live extraction-health | 🔴 Latest real candidate run **35310771629** tested older PR head `dbb3c08`: offline/metadata/search/related passed; `home-more` had a separate continuation parse failure; own-client/yt-dlp were runner bot-gated; NewPipe was a short-JSON parse watch. Final head `6dd98fb` has not had a live run. | S1 remains open. Owner must run the final head, then accept `home-more` only on a live pass and keep any `ENVIRONMENT_BLOCKED`/`UNAVAILABLE` resolver result separate. Do not begin S2. |
...
- [ ] **Final candidate validation pending — branch `arena/01a0b224-dhun@6dd98fb`:** PR #91 is CI-green and `CLEAN` (push CI **35313596684**, PR CI **35313601849**, Build APK **35313601854**, test-release **35313601903**), but no owner-triggered `extraction-health` run has tested this final head. Android and Windows/Desktop production paths are not being reopened. S1 remains RED until `home-more` passes live.
...
- [ ] **Home parser follow-up pending live validation:** the final candidate at `6dd98fb` contains the scoped direct/nested-browse parser support and classifier changes, all CI-green. Do not mark `home-more` green until an owner-triggered probe on `6dd98fb` passes; the older run `35310771629` is evidence only for `dbb3c08`.
