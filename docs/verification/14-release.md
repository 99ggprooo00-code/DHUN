...
> **Current status (2026-09-18 ~04:17 UTC, Stage S1 RED):** The historical body below is retained, but the current release baseline is `main@33e94b0` after PR #90. Main CI **35246193151**, Build APK **35246193174**, and test-release **35246193097** pass; the rolling `test` release contains APK/MSI and both checksum sidecars.
> Owner-dispatched workflow **35306224822** for active workflow **360655315 `extraction-health`** completed **failure** on `main@33e94b0` and uploaded `rot-drill-35306224822` (artifact id **10532130174**, 4,357 B). Offline/metadata/search/related checks passed, but `home-more` returned `Parse(detail=Home response contained no section list or Home continuation action)`; own-client and yt-dlp were bot-gated with `AuthRequired`, and NewPipe reported `Parse(detail=JSON response is too short)`. The result is a mixed RED, not a GREEN S1 exit; no live audio bytes were validated.
> PR **#91**'s parser evidence head is `005b526`; CI **35307916827**, Build APK **35307916736**, and test-release **35307916668** pass (push CI **35307913766**). The PR remains open and unmerged; this follow-up status commit is docs-only. The candidate is still not live-validated. The agent cannot dispatch workflows (HTTP 403), but the owner-triggered run is authoritative. S1 remains open: capture/reconcile the Home continuation shape and perform approved residential/device verification; do not add credentials, cookies, PO tokens, BotGuard, attestation, or ADR-007. No v0.1.0 tag/release or hardware acceptance is claimed.
> A narrow Home parser candidate is now present on PR #91: shelf-specific continuation wrappers/renderers are covered by synthetic tests, and unmatched-shape diagnostics report field names only. It is **not** a live acceptance result until CI and an owner-triggered candidate-branch probe or sanitized response confirm the shape.
> Owner run **35308796439** on candidate `c546d7b` completed the live probe and kept `home-more` **RED**: the response shape was `top[contents,responseContext,trackingParams]` with no action/continuation wrapper. The first candidate was therefore insufficient. A direct-contents follow-up is pending CI and another candidate-branch run.
> Follow-up head `526e390` is PR-green (CI **35309128614**, Build APK **35309128640**, test-release **35309128612**). Independent push CI **35309124090** flaked on an unrelated `LibraryViewModelTest` timeout; push CI **35309533562** then passed on docs-only successor `98843af`, returning PR #91 to `CLEAN`. This does not establish a parser failure.
